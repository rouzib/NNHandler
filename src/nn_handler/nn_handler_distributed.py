...
class NNHandler:
    ...
    def __init__(self,
        ...
        self._distributed = self._should_use_distributed(use_distributed)
        if self._distributed:
            self._initialize_distributed()  # Sets self._rank, self._local_rank, self._world_size, self._device
        else:
            self._rank = 0
            self._local_rank = -1  # Convention for non-distributed or CPU
            self._world_size = 1
            self._device = self._resolve_device(device)  # Resolve device if not DDP
    ...
    def _should_use_distributed(self, use_distributed_flag: Optional[bool]) -> bool:
        """Determines if DDP should be used based on flag and environment."""
        env_is_distributed = self._is_env_distributed()
    ...
    def device(self, value: Union[torch.device, str]):
        ...
        if self._distributed:
            resolved_value = self._resolve_device(value)
        ...
        resolved_device = self._resolve_device(value)
    ...
    def get_samples(self, N, device=None):
        ...
        target_device = self.device if device is None else self._resolve_device(device)
    ...
    def _prepare_batch(self, batch: Any) -> dict[str, Any | None]:
        ...
        if isinstance(batch, (list, tuple)):
            ...
            # Consider only warning or making it configurable
            # if num_extra_items_given < num_extra_params_needed:
    ...
    def load(path: str,
        ...
        if device is None:
            ...
            map_location_device = NNHandler._resolve_device(None, device)  # Use static method context
            map_location = str(map_location_device)  # torch.load needs string or torch.device

        # Use static method _resolve_device safely
        # Cannot call instance method _resolve_device before instance exists
        temp_device_resolver = NNHandler(model_class=nn.Module,
                                         use_distributed=False)  # Dummy instance for method access
        resolved_map_location_device = temp_device_resolver._resolve_device(map_location)
        del temp_device_resolver
    ...
    def initialize_from_checkpoint(checkpoint_path: str,
        ...
        if device is None:
            ...
            temp_device_resolver = NNHandler(model_class=nn.Module, use_distributed=False)
            resolved_map_location_device = temp_device_resolver._resolve_device(device)
            del temp_device_resolver
    ...
    # load_model_code static method remains the same as in nn_handler.py
    @staticmethod
